...
\pagestyle{fancy}
\fancyhead[L]{\scriptsize \textsc{Réalisation d'un laser à \lmbd{532} par génération de seconde harmonique}}
...
\parbox{15cm}{
\textbf{Mots clefs} : \it optique non-linéaire, génération de seconde harmonique (SHG), niobate de lithium polarisé périodiquement (PPLN), fibre optique, fibre à cristaux photoniques}%TODO
...
%Le refroidissement d'atomes jusqu'à la dégénérescence quantique permet l'étude expérimentale de systèmes quantiques à plusieurs corps. 
...
\begin{figure}[htbp] 
	...
	%\includepdf[pages=-,frame=true,templatesize={145mm}{210mm},pagecommand={\begin{tikzpicture}[remember picture, overlay]\node at (current page.center) {overlayed text};\end{tikzpicture}}, height=9cm]{img/accordeon-dmd.pdf}
...
\end{overpic}
	...
	\begin{subfigure}[b]{0.48\textwidth}
   	...
   	\input{img/transitions.tex}
		%}
		%\includegraphics[height=9cm]{transitions.pdf}
...
% Les atomes étant attirés par les minima d'intensité, on arrive à confiner les atomes dans un plan et les soumettre à un potentiel contrôlé (ici une ``boîte'' circulaire)}
\end{figure}
...
Pour un atome à deux niveaux avec une transition à $\omega_0$, dans l'approximation de faible saturation (état excité faiblement peuplé) et de fort désaccord ($\Delta = \omega - \omega_0$ vérifiant $|\Delta| \ll \omega_0$), on trouve %TODO
\vspace{-0.2cm}
\begin{align}
...
% de tester une fibre à cristaux photoniques
...
\begin{subfigure}[t]{0.48\textwidth}
	...
	\captionsetup{width=5.7cm}%margin={1cm,1cm}}
...
\hspace*{0.4cm}
\begin{minipage}[t]{0.48\textwidth}
    \include{./img/waist_faisceau_incident.tex}
		\vspace{-1cm}
...
%\footnotetext[\numexpr\value{footnote}-1]{Le terme ``puissance doublée'' est synonyme de ``puissance de la seconde harmonique''.}
\footnotetext{Pour les quelques températures avec une seule mesure, la barre d'erreur est reprise de la température précédente.}
...
Pour ce qui est de $\pdv{\left(\dke\right)}{T}$ (et donc de$\pdv{\left(n_2-n_1\right)}{T}$), déterminé par le rapport entre les largeurs de $h(a,b)$ (à $a$ fixé) et de $\alpha(T)$, l'ajustement donne $\pdv{\left(\dke\right)}{T}=\SI{1.7}{\per\centi\meter\per\kelvin}$ %(pour les deux valeurs de $\zr$, en accord avec la relation de proportionnalité approximative entre la largeur $\delta b$ de $h$ et $1/a = 
...
\end{tabular}
\caption{Efficacités mesurées à basse et haute puissance et température de l'optimum de doublage. \small Du fait du chauffage par absorption, la température optimale (du four) est plus basse à haute puissance. Le doublage à haute puissance est instable.} %TODO
\label{table:mes}
\end{table}
...
Enfin, puisqu'une température plus élevée semble améliorer la stabilité, j'ai essayé la bande $\Lambda=\SI{6.83}{\micro\meter}$.% (cf. la dernière du tableau \ref{table:mes}). 
...
%Pour ce qui est du but éducatif du stage, 
...
%En injectant l'expression du terme quadratique de la polarisation \eqref{eq:pola} dans l'équation de Helmholtz paraxiale \eqref{eq:parax}, on trouve, en plus de l'équation 
