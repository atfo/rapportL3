...
\newcommand{\alphae}[1]{\SI{5.5}{\percent W^{-1}}}
...
\begin{enumerate}
	\item On choisit une hauteur de faisceau avec laquelle on va trailler, et on align la source laser et les optiques à cette hauteur.
	\item À l'aide des vis des 2 miroirs règlant l'angle vertical, on assure l'horizontalité et la bonne hauteur du faisceau par rapport au collimateur en faisant passer le faisceau à travers ce dernier, en le plaçant en alternance près et loin des miroirs.
	\item On connecte la fibre au collimateur. Si l'alignement est bon, on doit avoir une puissance mesurable en sortie.
	\item On étudie le profil du faisceau en sortie de sorte à déterminer son waist. Puisque les collimateurs aux deux extrémités sont identiques, il s'agit du bon waist pour le faisceau incident.
	\item On monte un téléscope avec un grossissement permettant d'obtenir le waist voulu.
...
dans le domaine de Fourier en temps avec la convention $\v E(\v r, t) = \mathfrak{Re} \left\{ \sum_{n \in \mathbb N} \v {\boldsymbol{\mathcal E}}_q (\v r) \e{-i \omega_q t} \right\}$,
...
\end{subfigure}
\hspace*{-0.6cm}
\caption{Amplitude de seconde harmonique et quasi-accord de phase (TODO légende+alignement)} %TODO
\label{fig:QPM}
\end{figure}
...
Nous avons choisi une autre solution qui consiste à fabriquer un cristal dont le $\chi^{(2)}$ varie spatialement. En effet, si $\chi^{(2)}(z) = \chi^{(2)}_0 \e{i k_\chi z}$, cela revient à remplacer $\Delta k$ par $\Delta k_\mathsc{eff} = \Delta k - k_\chi$ dans (\ref{eq:pwe}). 
...
Évidemment, la susceptibilité est en réalité proche d'une valeur réelle et un tel $\chi^{(2)}$ n'est pas réalisable. À la place, on inverse le signe de $\chi^{(2)}$ en inversant l'axe extraordinaire d'un matériau ferroélectrique avec une période $\Lambda$ (cf figure \ref{fig:QPM}). 
...
\begin{align}
	...
	\text{ou } i &\frac{\chi_\mathsc{eff}\omega}{2 cn_2} \A_1^2 L \operatorname{sinc}\left( \frac{\Delta k_\mathsc{eff} L}{2} \right) 
...
\begin{align}
	...
	\text{soit } \alpha &= \frac{\mathcal P_2}{\mathcal P_1^2} = \frac{\chie^2 \omega^2 L^2}{2 \varepsilon_0 c^3 \pi w_0^2 n_1^2 n_2}
\end{align}
...
La théorie de Boyd-Kleinman consiste donc à poser l'Ansatz suivant $$\A_2(r,z) = \frac{A_2(z)}{1+i\zeta}\e{-\frac{2r^{2}}{w_{0}^{2} (1+i\zeta) }} $$où l'on a toujours $\zeta = (z\lambda_1)/(n_1 \pi w_{0}^{2})$. % Cet Ansatz peut aussi être vu comme une variation de la constante dans la solution de l'équation linéaire (sans ``terme source''). (Pas vrai à cause de n_1 au lieu de n_2!)
En injectant l'Ansatz dans (\ref{eq:SHG}) mais avec les quantités effectives du quasi-accord de phase et en négligeant le terme en $\Delta k \ll k_2$, on trouve que $A_2$ vérifie l'équation d'évolution suivante :
\begin{align} 
	\dv{A_2}{z} = \frac{i\omega \chi_\mathsc{eff}}{2 n_2 c} A_1^2 \frac{\e{-i\Delta k_\mathsc{eff} \zr \zeta}}{1+i\zeta} 
	\label{eq:BKDE}
\end{align}

On remarque au passage que Boyd et Kleinman ont fait le choix de prendre un waist exactement $\sqrt{2}$ fois plus petit, de sorte que la relation entre waist et longueur de Rayleigh d'un faisceau gaussien n'est vérifiée qu'approximativement, ce qui a conduit au terme parasite en $\Delta k$ qu'il a fallu négliger. Le choix alternatif de prendre le ``bon'' waist n'est pas plus satisfaisant car il conduit à un facteur $\e{ \left(\frac{n_2}{n_1}-1\right) \frac{2 r^2}{w_0^2(1+i\zeta)}}$ parasite dans le second membre. Quoi qu'il en soit, nous sommes surtout intéressés par l'expression de la puissance de vert générée, et cette dernière peut être obtenue sans hypothèse sur le profil du faisceau, comme nous le montrerons en annexe \ref{BK}. 
...
\begin{align*} 
	...
	&= \frac{\omega^3 \chie^2}{2 \varepsilon_0 c^4 \pi n_1 n_2} \zr 
	\left| \int_{-L/2\zr}^{L/2\zr} \frac{\e{-i\Delta k_\mathsc{eff} \zr \zeta}}{1+i\zeta} \mathrm d\zeta \right|^2\\
...
Notons au passage que l'on retrouve bien le résultat pour les ondes planes dans la limite $a\ll1$.
...
Le profil de $h$ est tracé figure \ref{fig:bk-factor} et sera discuté plus bas en relation avec la réalisation expérimentale du doublage.
...
\begin{figure}[h]
	\centering
	\includegraphics[height=10cm]{./img/schema.pdf}
	%\include{./img/schema.tex}
...
En effet, le tracé de $\alpha$ en fonction de la température correspond, modulo la variation du préfacteur $\frac{\chie^2 L}{n_1 n_2}$, à la variation de $h(a,b)$ avec $a$ fixé et $b=-\Delta_\mathsc{eff} \zr$ variant avec les indices optiques. Si on linéarise la variation d'indices optiques au voisinage de l'optimum de température, on a alors 
$$\delta b = b \left( -\frac{\pdv{n_2-n_1}{T}}{\frac{\lambda_2}{\Lambda} - (n_2-n_1)} + \pdv{n_1}{T} \right) \delta T \approx {\color{red} 0.1} b \delta T$$ 
et les graphes de $\alpha(T)$ et de $h(a,b)$ à $a$ fixé (figure \ref{fig:bk-factor}) sont reliés par dilatation des deux axes.
